...
### GitHub Checks (PR commit status) + sticky PR comment

Posts a check run on a PR's head commit reflecting the run outcome, and — when the run carries a PR number — upserts a single **sticky PR comment** summarizing failures (split new vs. already-failing on the previous run), flaky detections, and deep links into the dashboard. **All-or-nothing**: enabled only when `GITHUB_APP_ID` + `GITHUB_APP_PRIVATE_KEY` + `GITHUB_APP_WEBHOOK_SECRET` are all set. This is a GitHub **App** (mints an installation token that works on fork PRs) — distinct from the `AUTH_GITHUB_*` OAuth creds used for sign-in. In the App's settings, set the **Webhook URL** to `${WRIGHTFUL_PUBLIC_URL}/api/github/webhook` and the **Webhook secret** to your `GITHUB_APP_WEBHOOK_SECRET`; the route 404s until all three `GITHUB_APP_*` secrets are set. Grant the App repository permissions **Checks: Read & write** (check runs) and **Pull requests: Read & write** (the sticky comment); without the latter, comment posts fail with 403 — logged, never failing ingest. With the App configured, prefer it over the reporter's CI-token `postPrComment` fallback: the two use distinct markers, so enabling both stacks two comments per PR.
...
**The default is safe without any configuration**, and rests on two things:

1. **The dashboard's own replay pane drops `allow-scripts`** on same-origin snapshot iframes, so no snapshot script executes. That sandbox flag is the **enforcing** control, and it is deliberately the only thing this rests on — the `script-src 'none'` CSP the worker attaches to `/trace-viewer/snapshot/*` is defence in depth that does **not** reach the snapshot document, because Playwright's service worker synthesises that response locally and it never traverses the worker.
2. **Playwright's own HTML shells are not shipped at all.** `/trace-viewer/index.html` (the stock viewer SPA), `snapshot.html` (the popout target) and `uiMode.html` each frame snapshots with a **hardcoded** `sandbox="allow-same-origin allow-scripts"` that cannot be overridden. Unless you configure a cookieless viewer origin (below), `scripts/vendor-trace-viewer.mjs` **does not vendor them**, so all three 404. That is a build-time decision on purpose: these are static assets, and static assets are served _without_ running the worker's middleware, so "not on disk" is the only enforceable answer — a runtime check would never execute. The **"open snapshot in a new tab"** popout and the MCP `traceViewerUrl` field are gated on the same condition, so nothing links a file that isn't there. `npx playwright show-trace <downloadUrl>` is the safe alternative: it renders on your own localhost, which holds no dashboard session.

The cost is fidelity. The static DOM renders, but every fixup Playwright's blocked bootstrap would have applied is missing: **scroll position, input/checkbox/select values, shadow roots, nested frames, canvas contents, constructed (adopted) stylesheets, custom-element registration, open `<dialog>`/popover state, the acted-element highlight, and the click-point marker**. The dashboard re-applies exactly two things from the parent frame — it lifts Playwright's visibility guard so the DOM is not a blank white pane, and it suppresses link clicks so the snapshot cannot navigate itself (the serializer leaves `href` pointing at the original live URL). Screenshots, the timeline, and every non-snapshot tab are unaffected.

To get **full-fidelity replay** back safely, serve the trace-viewer scope from a **separate, cookieless origin** and set `VITE_WRIGHTFUL_TRACE_VIEWER_ORIGIN` to it (a **build-time** variable — it is inlined into the client bundle, so set it before `pnpm deploy:cf` / the Workers Build, alongside the `CF_*` vars). When set, snapshot iframes become cross-origin to the session, so `allow-scripts` is re-enabled with no access to your cookies/DOM. Setting it also re-enables the three HTML shells, the popout, and the MCP `traceViewerUrl`.

> **Residual risk once configured.** The same Worker serves both hostnames, so those shells are then reachable on the dashboard hostname too, where their hardcoded `allow-scripts` _is_ a session origin. Nothing links them there (every link is built on the viewer origin), but they are fetchable by URL. If that matters to you, block `/trace-viewer/index.html`, `/trace-viewer/snapshot.html` and `/trace-viewer/uiMode.html` on the dashboard hostname at your edge — the worker cannot do it, since static assets bypass its middleware.
