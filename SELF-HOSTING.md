...
2. **Playwright's own HTML shells are refused off the viewer origin.** `/trace-viewer/index.html` (the stock viewer SPA), `snapshot.html` (the popout target) and `uiMode.html` each frame snapshots with a **hardcoded** `sandbox="allow-same-origin allow-scripts"` that cannot be overridden, so the worker returns **404** for all three on any origin that is not a configured cookieless viewer host. They stay vendored because they _are_ the full-fidelity viewer on that host. This is a refusal, not merely a hidden link: those are static files, so anyone could otherwise reach them by typing the URL. The **"open snapshot in a new tab"** popout and the MCP `traceViewerUrl` field are correspondingly absent in same-origin mode — `npx playwright show-trace <downloadUrl>` is the safe alternative, since it renders on your own localhost, which holds no dashboard session.
