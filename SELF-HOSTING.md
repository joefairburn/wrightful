...
**The default is safe without any configuration:** same-origin snapshot iframes drop `allow-scripts`, so no snapshot script executes. That sandbox flag is the **enforcing** control, and it is deliberately the only thing the safety of the default rests on — the `script-src 'none'` CSP the worker attaches to `/trace-viewer/snapshot/*` is defence in depth that does **not** apply to the snapshot document itself, because Playwright's service worker synthesises that response locally and it never traverses the worker. The same reasoning removes the **"open snapshot in a new tab"** popout in same-origin mode: it renders through Playwright's vendored `snapshot.html`, whose iframe hardcodes `allow-scripts`, so the control only appears once a separate viewer origin is configured.
