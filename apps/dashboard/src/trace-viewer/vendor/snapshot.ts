...
// packages/trace/src/snapshot.ts — verbatim except the import of
// `Entry as HAREntry` from './har' (unchanged path, just now pointing at our
// vendored har.ts alongside it). Pulled in because ./trace.ts and
// ./entries.ts both need `ResourceSnapshot` / `FrameSnapshot`.
