...
/* oxlint-disable typescript-eslint/no-explicit-any -- vendored verbatim from upstream; kept byte-diffable against microsoft/playwright rather than restyled */
// VENDOR-PROVENANCE: microsoft/playwright, tag v1.61.1,
// packages/trace/src/trace.ts — verbatim trace-event wire types (VERSION 8).
// Only the import paths are adapted to point at sibling files in this
// vendor/ folder instead of the monorepo's `@isomorphic` / `@protocol`
// aliases:
//   './snapshot'                       (was './snapshot', unchanged)
//   '@isomorphic/locatorGenerators'  -> './language' (Language type only)
//   '@protocol/channels'             -> './protocol-types' (Point,
//                                        SerializedError, StackFrame only)
// No runtime logic in this file (types only) — nothing else changed.

import type { FrameSnapshot, ResourceSnapshot } from "./snapshot";
import type { Language } from "./language";
import type { Point, SerializedError, StackFrame } from "./protocol-types";
