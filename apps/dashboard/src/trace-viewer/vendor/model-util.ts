...
/* oxlint-disable no-underscore-dangle, no-shadow, typescript-eslint/no-explicit-any, typescript-eslint/no-unsafe-type-assertion -- vendored verbatim from upstream; kept byte-diffable against microsoft/playwright rather than restyled */
// VENDOR-PROVENANCE: microsoft/playwright, tag v1.61.1,
// packages/isomorphic/trace/traceModel.ts.
//
// The task/brief that motivated this vendoring pass named this file
// `packages/trace-viewer/src/ui/modelUtil.ts`, exporting a `MultiTraceModel`
// class — that is the PRE-refactor location and name. As of v1.61.1,
// `trace-viewer/src/ui/modelUtil.ts` no longer exists; the class was moved
// (and renamed to `TraceModel`) into the shared `packages/isomorphic`
// package specifically so non-UI consumers (like the trace-viewer service
// worker itself) can construct/use it without pulling in React. That move
// makes this file naturally UI-free already — nothing needed trimming.
//
...
//   '../locatorGenerators'        -> './language'            (Language only)
//   '@trace/snapshot'             -> './snapshot'
//   '@trace/trace'                -> './trace'
//   './entries'                   -> './entries'              (unchanged)
//   '@protocol/channels'          -> './protocol-types'        (StackFrame only)

import { getActionGroup, renderTitleForCall } from "./protocol-formatter";

import type { Language } from "./language";
import type { ResourceSnapshot } from "./snapshot";
import type * as trace from "./trace";
import type { ActionTraceEvent } from "./trace";
import type { ActionEntry, ContextEntry, PageEntry } from "./entries";
import type { StackFrame } from "./protocol-types";
import type { ActionGroup } from "./protocol-formatter";
...
export class TraceModel {
  ...
  constructor(traceUri: string, contexts: ContextEntry[]) {
    ...
    const libraryContext = contexts.find(
      (context) => context.origin === "library",
    );
    ...
    this.playwrightVersion = contexts.find(
      (c) => c.playwrightVersion,
    )?.playwrightVersion;
    this.title = libraryContext?.title || "";
    this.options = libraryContext?.options || {};
    this.testTimeout = contexts.find(
      (c) => c.origin === "testRunner",
    )?.testTimeout;
    ...
    this.wallTime = contexts
      .map((c) => c.wallTime)
      .reduce(
        (prev, cur) => Math.min(prev || Number.MAX_VALUE, cur!),
        Number.MAX_VALUE,
      );
    ...
    this.events = (
      [] as (trace.EventTraceEvent | trace.ConsoleMessageTraceEvent)[]
    ).concat(...contexts.map((c) => c.events));
    this.stdio = ([] as trace.StdioTraceEvent[]).concat(
      ...contexts.map((c) => c.stdio),
    );
    this.errors = ([] as trace.ErrorTraceEvent[]).concat(
      ...contexts.map((c) => c.errors),
    );
    this.hasSource = contexts.some((c) => c.hasSource);
    this.hasStepData = contexts.some(
      (context) => context.origin === "testRunner",
    );
    this.resources = [...contexts.map((c) => c.resources)]
      .flat()
      .map((entry) => ({
        ...entry,
        id: `${entry.pageref}-${entry.startedDateTime}-${entry.request.url}`,
      }));
    this.attachments = this.actions.flatMap(
      (action) =>
    ...
    this.visibleAttachments = this.attachments.filter(
      (attachment) => !attachment.name.startsWith("_"),
    );
    ...
    for (const action of this.actions) {
      action.group =
        action.group ??
        getActionGroup({ type: action.class, method: action.method });
      if (action.group)
        this.actionCounters.set(
          action.group,
          1 + (this.actionCounters.get(action.group) || 0),
        );
    }
  }
  ...
  filteredActions(actionsFilter: ActionGroup[]) {
    ...
    return this.actions.filter(
      (action) => !action.group || filter.has(action.group),
    );
  }
  ...
  renderActionTree(filter?: ActionGroup[]) {
    ...
    const visit = (actionItem: ActionTreeItem, indent: string) => {
      const title = renderTitleForCall({
        ...actionItem.action,
        type: actionItem.action.class,
      });
      actionTree.push(`${indent}${title || actionItem.id}`);
      for (const child of actionItem.children) visit(child, indent + "  ");
    };
  ...
  private _errorDescriptorsFromActions(): ErrorDescription[] {
    ...
    for (const action of this.actions || []) {
      if (!action.error?.message) continue;
      errors.push({
        action,
        stack: action.stack,
...
function indexModel(context: ContextEntry) {
  ...
  for (let i = context.actions.length - 1; i >= 0; i--) {
    ...
    if (action.class !== "Route") lastNonRouteAction = action;
  }
...
function mergeActionsAndUpdateTimingSameTrace(
  contexts: ContextEntry[],
): ActionTraceEventInContext[] {
  const map = new Map<string, ActionTraceEventInContext>();

  const libraryContexts = contexts.filter(
    (context) => context.origin === "library",
  );
  const testRunnerContexts = contexts.filter(
    (context) => context.origin === "testRunner",
  );
  ...
  for (const context of libraryContexts) {
    for (const action of context.actions) {
      ...
      map.set(action.stepId || `tmp-step@${++lastTmpStepId}`, {
        ...action,
        context,
      });
    }
  ...
  // runner steps.
  const delta = monotonicTimeDeltaBetweenLibraryAndRunner(
    testRunnerContexts,
    map,
  );
  if (delta) adjustMonotonicTime(libraryContexts, delta);
  ...
  for (const context of testRunnerContexts) {
    for (const action of context.actions) {
      ...
      if (existing) {
        ...
        if (action.error) existing.error = action.error;
        if (action.attachments) existing.attachments = action.attachments;
        if (action.annotations) existing.annotations = action.annotations;
        if (action.parentId)
          existing.parentId =
            nonPrimaryIdToPrimaryId.get(action.parentId) ?? action.parentId;
        if (action.group) existing.group = action.group;
...
function adjustMonotonicTime(
  contexts: ContextEntry[],
  monotonicTimeDelta: number,
) {
  for (const context of contexts) {
    ...
    for (const action of context.actions) {
      ...
      if (action.endTime) action.endTime += monotonicTimeDelta;
    }
    for (const event of context.events) event.time += monotonicTimeDelta;
    for (const event of context.stdio) event.timestamp += monotonicTimeDelta;
...
function monotonicTimeDeltaBetweenLibraryAndRunner(
  nonPrimaryContexts: ContextEntry[],
  libraryActions: Map<string, ActionTraceEventInContext>,
) {
  ...
  for (const context of nonPrimaryContexts) {
    for (const action of context.actions) {
      if (!action.startTime) continue;
      const libraryAction = action.stepId
        ? libraryActions.get(action.stepId)
        : undefined;
      if (libraryAction) return action.startTime - libraryAction.startTime;
    }
  }
  return 0;
}

export function buildActionTree(actions: ActionTraceEventInContext[]): {
  ...
  const rootItem: ActionTreeItem = {
    action: { ...kFakeRootAction },
    id: "",
    parent: undefined,
    children: [],
  };
  for (const item of itemMap.values()) {
    rootItem.action.startTime = Math.min(
      rootItem.action.startTime,
      item.action.startTime,
    );
    rootItem.action.endTime = Math.max(
      rootItem.action.endTime,
      item.action.endTime,
    );
    const parent = item.action.parentId
      ? itemMap.get(item.action.parentId) || rootItem
      : rootItem;
...
export function context(
  action: ActionTraceEvent | trace.EventTraceEvent | ResourceSnapshot,
): ContextEntry {
...
export function previousActionByEndTime(
  action: ActionTraceEvent,
): ActionTraceEvent {
  return (action as any)[prevByEndTimeSymbol];
}

export function nextActionByStartTime(
  action: ActionTraceEvent,
): ActionTraceEvent {
  return (action as any)[nextByStartTimeSymbol];
}

export function stats(action: ActionTraceEvent): {
  errors: number;
  warnings: number;
} {
  let errors = 0;
  ...
  for (const event of eventsForAction(action)) {
    ...
    if (event.type === "event" && event.method === "pageError") ++errors;
  }
...
export function eventsForAction(
  action: ActionTraceEvent,
): (trace.EventTraceEvent | trace.ConsoleMessageTraceEvent)[] {
  let result: (trace.EventTraceEvent | trace.ConsoleMessageTraceEvent)[] = (
    action as any
  )[eventsSymbol];
  ...
  result = context(action).events.filter((event) => {
    return (
      ...
      (!nextAction || event.time < nextAction.startTime)
    );
  });
