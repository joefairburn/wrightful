...
**Protocol versioning**: `X-Wrightful-Version` header. Currently only version 3 is supported — older reporters/CLIs get a 409. Applies to INGEST only — the read surfaces (`/api/v1/*`, `/api/mcp`) are versionless Bearer-only contracts (MCP negotiates its own protocol version inside JSON-RPC).
...
- **Design conventions** (2026-07 consistency pass — see `docs/worklog/2026-07-06-design-system-consolidation.md`):
  ...
  - **Micro-labels** (table headers, KPI/section/meta labels) are `text-caption font-medium tracking-[0.1px] text-fg-3` (the 12px ramp step), normal case — never uppercase/tracking-wide. `TableHead` bakes it in; don't restyle header cells.
...
Plus one hybrid: **`/api/mcp`** accepts a project API key OR a user-scoped OAuth access token minted by the Better Auth `mcp` plugin (registered in `auth.ts`; discovery documents rewritten to root `/.well-known/*` in `void.json`; consent screen at `pages/oauth/consent.tsx`, FORCED via `forceConsentRedirect` in `middleware/02.api-auth.ts` — do not remove that leg, the plugin auto-grants without it). OAuth-authed tools take `team`/`project` args membership-checked per call. See `docs/api/mcp.md` + `docs/worklog/2026-07-05-mcp-server.md`.
...
- `requireTenantContext(c)` — `apps/dashboard/src/lib/tenant-context.ts` — returns `{ project, scope }` from the active project resolved once by `middleware/01.context.ts`. **Use this at the top of any `/t/:teamSlug/p/:projectSlug/*` page loader** — it avoids re-running the membership join.
...
- **⚠️ Before every `git commit`: run `pnpm check` (exit 0) AND let the pre-commit hook run — do NOT pass `--no-verify`.** The `.vite-hooks/` pre-commit hook (`vp staged`) works fine in Conductor git worktrees — it auto-runs `fmt --write` + `lint --fix` on staged files and re-stages them, so there's no reason to bypass it (any "could not write index" worktree failure no longer reproduces). But `vp staged` does **not** type-check, so a type error (`TS####`) still slips past the hook. Run `pnpm check` yourself (format + lint + type-check in one pass), confirm exit 0, plus the relevant `pnpm --filter … test` for code you changed, and only then commit. Don't commit-then-let-CI-find-it.
