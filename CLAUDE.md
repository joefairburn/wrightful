...
- **Design conventions** (2026-07 consistency pass — see `docs/worklog/2026-07-06-design-system-consolidation.md`):
  ...
  - **Shared chrome, not bespoke markup**: status chips → `<StatusPill>` (via `StatusBadge`/`MonBadge`), neutral mono meta chips → `META_PILL_CLASSES`, underline tabs → `underlineTabClasses()`, clickable table rows → `<RowLink>`, filter dropdowns → `MultiComboboxFilter`/`ComboboxFilterPopup`, danger confirms → `DANGER_TRIGGER_CLASSES`, empty states → `ui/empty`, list footers → `TablePaginationFooter` (pagination props optional).
