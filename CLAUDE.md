...
- **`packages/cli`** — Node CLI (`wrightful upload <report>`). Built with Commander, tsup. Parses Playwright JSON reports, detects CI env, uploads to dashboard API.
...
pnpm --filter @wrightful/cli build      # cli (tsup)
