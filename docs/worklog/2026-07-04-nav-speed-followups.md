...
| `monitors/[monitorId]/index.server.ts` (detail) | ~5 serialized phases: `getMonitor` → `listExecutions` → `httpUptimeWindows` → `httpResponseTimeBuckets` → `members`/`groups` | one `all({ monitor, executions, windows, responseRows, members, groups })` — `executions` runs concurrently with the 404 gate; the analytics + owner-picker reads `await this.$.monitor` (for type/teamId) then run in parallel. Throwing 404 inside the `monitor` task aborts siblings (`this.$signal`) and rejects the batch. |
